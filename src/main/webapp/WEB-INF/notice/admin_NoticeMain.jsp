...
// 			console.log('전달받은 url값은? ',res);
			$("div#into_notice_list").html(res);
			
			$('div.noticeAdminBtn').children('div.n_a_list').addClass('selected_not_menu')
		},error: function(request, status){
			console.log('인서트가져오기에러');
		}
	});
});
...
$(document).on('click', '.n_a_button', function(){
	$('div.n_a_button').removeClass("selected_not_menu");
	$(this).addClass("selected_not_menu");
});
...
<div class="container noticeWrap_admin">
		...
		<div class="container" id="noticeSearchDiv_admin">
		
			<div class="row noticeAdminBtn">
				<div class="col-2 n_a_button n_a_list" onClick="getlistNotice()">목록보기</div>
				<div class="col-2 n_a_button n_a_write" onClick="getInsertNotice()">글쓰기</div>
			</div>
		</div>
