<%@ page language="java" contentType="text/html; charset=UTF-8"
         pageEncoding="UTF-8" %>
<%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core"%>
<%@ include file="../../header.jsp" %>
<%
String loginError = request.getParameter("loginError"); // 권한 없을 때 뜨는 alert(※0512)
if(loginError!=null){
    out.println("<script>");
    out.println("if (window.history.replaceState) {");
    out.println("    window.history.replaceState(null, null, window.location.href.split('?')[0]);");
    out.println("} else {");
    out.println("    window.location.href = window.location.href.split('?')[0];");
    out.println("}");
    out.println("alert('로그인이 필요한 서비스입니다.');");
    out.println("</script>");
}
%>

<body>
<script>
function getHosListCate(val){
	   if(val == '강아지') {
		   location.href = "getHosListUser?searchCondition="+val;
	   } else if(val == '고양이') {
		   location.href = "getHosListUser?searchCondition="+val;
	   } else if(val == '새') {
		   location.href = "getHosListUser?searchCondition="+val;
	   } else if(val == '파충류') {
		   location.href = "getHosListUser?searchCondition="+val;
	   } else if(val == '기타동물') {
		   location.href = "getHosListUser?searchCondition="+val; 
	   } else if (val == '전체동물') {
			location.href = "getHosListUser?searchCondition=" + val;
		}
}


</script>




<nav class="navbar" id="petmedicnavbar">
	<div id="navAll">
		<div>
			<div id="logoNHam">
				<!-- logo -->
				<div id="navbarlogo">
					<a href="index.jsp"> <img style="width: 120px;"
					src="${pageContext.request.contextPath}/img/PetMedicLogo_P.png"></a>
				</div>
				<a class="navbarHamBtn">
				<svg id="navbarBtnSvg" xmlns="http://www.w3.org/2000/svg" width="70" height="100" fill="#5D3891" class="bi bi-list" viewBox="0 0 16 16">
				  <path fill-rule="evenodd" d="M2.5 12a.5.5 0 0 1 .5-.5h10a.5.5 0 0 1 0 1H3a.5.5 0 0 1-.5-.5zm0-4a.5.5 0 0 1 .5-.5h10a.5.5 0 0 1 0 1H3a.5.5 0 0 1-.5-.5zm0-4a.5.5 0 0 1 .5-.5h10a.5.5 0 0 1 0 1H3a.5.5 0 0 1-.5-.5z"/>
				</svg>
				</a>
			</div>
			</div>
			
		</div>
</nav>

   <div id="navlogin">
         <ul class="nav">
            <c:choose>
               <c:when test="${users_id ne NULL}">
                  <li class="nav-item">
                     <a   href="tomypagemain" class="nav-link loginOK nameNavBar">${users_name}님</a>
                  </li>
                  <c:choose>
                     <c:when test="${kakao_login == 'true'}">
                     <li class="nav-item">
                        <a class="nav-link" href="#" onclick="location.href='https://kauth.kakao.com/oauth/logout?client_id=1a705d08cde8b1fe4e40ecb2ddbe8d45&logout_redirect_uri=http://www.petmedic.kro.kr/logout'">
<!--                         <a class="nav-link" href="#" onclick="location.href='https://kauth.kakao.com/oauth/logout?client_id=1a705d08cde8b1fe4e40ecb2ddbe8d45&logout_redirect_uri=http://localhost:8090/logout'"> -->
                  로그아웃</a>
                     </li>
                     </c:when>
                     <c:otherwise>
                     <li class="nav-item">
                        <a class="nav-link loginOK" href="logout">로그아웃</a>
                     </li>
                     </c:otherwise>
                  </c:choose>
               </c:when>
               <c:when test="${hos_role eq 'Y'}"> <!-- 병원회원일때 -->
                  <li class="nav-item">
                     <a href="toHosMyPage" class="nav-link loginOK nameNavBar">${hos_id}님</a>
                  </li>
                  <li class="nav-item">
                     <a class="nav-link loginOKLogOut" href="logout">로그아웃</a>
                  </li>
               </c:when>
               <c:otherwise>
                  <li class="nav-item">
                     <li class="nav-item"><a class="nav-link loginNO" href="beforeJoin">회원가입</a>
                  </li>
               </c:otherwise>
            </c:choose>
         </ul>
      </div>
		

<!-- Links -->
<div id="mainMenuListUlDiv">
	<ul class="nav mainMenuList">
		<li class="nav-item dropdown petMedicDrop">
			<div class="petMedicMenuList">
			<a class="dropdown-toggle nav-link petmedicnav-link petMedicDrop" id="petCate" data-toggle="dropdown">
			<b id="navb">동물카테고리</b>
			</a>
			</div>
			
			<div class="dropdown-container">
				<div id="petMedicDropMenu">
					<a class="dropdown-item dropMenus search-select " id="searchCondition" 
						onclick="getHosListCate('강아지')"><span id="dog" class="dropPet" >강아지</span></a>
					<a class="dropdown-item dropMenus  search-select " id="searchCondition" 
						onclick="getHosListCate('고양이')"><span id="cat" class="dropPet">고양이</span></a>
					<a class="dropdown-item dropMenus  search-select " id="searchCondition" 
						onclick="getHosListCate('새')"><span id="bird" class="dropPet">새</span></a>
					<a class="dropdown-item dropMenus  search-select " id="searchCondition" 
						onclick="getHosListCate('파충류')"><span class="dropPet">파충류</span></a>
					<a class="dropdown-item dropMenus  search-select " id="searchCondition" 
						onclick="getHosListCate('기타동물')"><span class="dropPet">기타동물</span></a>
				</div>
			</div>
		</li>			
	
		<li class="nav-item"><a href="searchHospitalList" class="nav-link petmedicnav-link"> <b id="navb">병원지도검색</b></a></li>

		<li class="nav-item"><a href="toGetNoticeList" class="nav-link petmedicnav-link"> <b id="navb">공지사항</b></a></li>

		<li class="nav-item"><a href="togetFaqList" class="nav-link petmedicnav-link"> <b id="navb">FAQ</b></a></li>
		
			<li class="nav-item"><a href="tomypagemain" class="nav-link petmedicnav-link"> <b id="navb">마이페이지</b></a></li>
	
<%-- 			<c:if test="${hos_role eq 'Y'}"> --%>
<!-- 			<li class="nav-item"><a href="toHosMyPage" class="nav-link petmedicnav-link"> <b id="navb">내 병원 페이지</b></a></li> -->
<%-- 			</c:if>	 --%>
			
	
	</ul>
</div>
			
 
<script>

(function () {
	$("#nologin").hide();
	   $("#nologin2").hide();
	   const mainHamtoggleBtn = document.querySelector("#navbarBtnSvg");
	   const menu = document.querySelector(".mainMenuList");
	   const loginMenu = document.querySelector("#navlogin");

	   //이건 햄벅어 메뉴 토글임
	   mainHamtoggleBtn.addEventListener("click", function() {
	      menu.classList.toggle("active");
	     loginMenu.classList.toggle("active");
	   });
	   
	   
	   //최초 1회 눌렀을 때 닫히는거 막아줌 
	   const dropBtn = document.querySelector("#petCate");
	   const dropMenu = document.getElementById("petMedicDropMenu");

	   dropBtn.addEventListener("click", function (event) {
	     event.stopPropagation(); // 이벤트 전파 중지
	     
	     if (!dropMenu.classList.contains("active")) {
	       dropMenu.classList.add("active"); // 드롭다운 메뉴 열기
	     } else {
	       dropMenu.classList.remove("active"); // 드롭다운 메뉴 닫기
	     }
	   });

	   document.addEventListener("click", function (event) {
	     const target = event.target;

	     if (
	       !target.matches("#navbarBtnSvg") &&
	       !target.matches(".mainMenuList") &&
	       !target.matches("#navlogin") &&
	       !target.matches("#petMedicDropMenu")
	     ) {
	       dropMenu.classList.remove("active"); // 드롭다운 메뉴 닫기
	     }
	   });
	      
	   
	   
	   //드롭다운 메뉴에 호버했을 때 드롭다운 닫혀요이
	   const dropList = document.querySelectorAll(".dropMenus");

	   dropList.forEach(function (dropListItem) {
	     const dropSpans = dropListItem.querySelectorAll(".dropPet");

	     dropListItem.addEventListener("mouseover", function (event) {
	       dropSpans.forEach(function (dropSpan) {
	         dropSpan.classList.add("dropHover");
	       });
	     });

	     dropListItem.addEventListener("mouseout", function (event) {
	       dropSpans.forEach(function (dropSpan) {
	         dropSpan.classList.remove("dropHover");
	       });
	     });
	   });
	   
	   
	   
	})(); //즉시실행함수 끝   


//카카오로그인
function kakaoLogin() {
   var pathNamed = location.pathname;
   if (pathNamed == "/index.jsp") {
      pathNamed = "/";
   }
   pathNamed = pathNamed.substring(1) + location.search;
   pathNamed = encodeURIComponent(pathNamed)
   location.href='https://kauth.kakao.com/oauth/authorize?client_id=1a705d08cde8b1fe4e40ecb2ddbe8d45&state=' + pathNamed + '&redirect_uri=http://petmedic.kro.kr/kakaologin&response_type=code';
//    location.href='https://kauth.kakao.com/oauth/authorize?client_id=1a705d08cde8b1fe4e40ecb2ddbe8d45&state=' + pathNamed + '&redirect_uri=http://localhost:8090/kakaologin&response_type=code';
}


</script>

<div class="jumbotron">
</div>
<div class="container" align="center">
    <div class="col-md-4 col-md-offset-4">
        <h3 class="form-signin-heading">로그인</h3>
        
        
        <form class="form-signin" action="login2" method="post">
            <div class="form-group">
                <input type="text" class="form-control" id="inputUserName" placeholder="ID" name="users_id" required autofocus>
            </div>
            <div class="form-group">
                <input type="password" class="form-control" id="inputPassword" placeholder="Password" name="users_pw" required>
            </div>
            <c:if test="${not empty nologin}">
   <div class='alert alert-danger' id="nologin">
      아이디와 비밀번호를 확인해 주세요
   </div>
</c:if>
            <div class="text-right">
            <a href="forgotId" id="loginmodaltxt" class="seonchanaclass">아이디찾기</a> | <a href="resetPassword" id="loginmodaltxt" class="seonchanaclass">비밀번호 재설정</a>
        </div>
            <button class="btn btn btn-lg btn-success btn-block" id="navbarloginbt" type="submit">로그인</button>
        </form>
         
        
    </div>
</div>


<%@ include file="footer.jsp"%>

</body>
</html>