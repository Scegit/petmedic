	    ...
	    if (userInfo != null) {
	        if (usersService.pwMatchChk(user_pw, userInfo.getUsers_pw())) {
	            if (userInfo.getUsers_role().equals("USER")) {
	                ...
	                System.out.println("False: 올바른 비밀번호");
	            } else {
	                Login.put("loginError", "true");
	                System.out.println("True: 올바른 비밀번호, 권한 없음");
	            }
	        } else {
	            Login.put("loginError", "true");
	            System.out.println("True: 잘못된 비밀번호");
	        }
	    } else {
	        Login.put("loginError", "true");
...
//			String chkPassword = usersService.getUsers(vo).getUsers_pw();
//			System.out.println("비밀번호 확인 : " + chkPassword);
//
//			boolean chk = false;
//			if (user_pw != null) {
//				chk = usersService.pwMatchChk(user_pw, chkPassword);
//			}
//
//			if (chk) {
//				System.out.println("일치함");
//				session.setAttribute("login", usersService.getUsers(vo));
//				session.setAttribute("users_id", usersService.getUsers(vo).getUsers_id());
//				session.setAttribute("users_name", usersService.getUsers(vo).getUsers_name());
//				session.setAttribute("users_role", usersService.getUsers(vo).getUsers_role());
//				return "redirect:index.jsp";
//			} else {
